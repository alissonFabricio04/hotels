...
export default class GetHotelsByPeriod {
  ...
  async handle(capacity: number): Promise<Output> {
    const capacty = new Int(capacity)

    const rms = await this.roomRepository.getByRoomCapacity(capacty)
    ...
    for (let i = 0; i < rms.length; i++) {
      ...
      if (hotels.length === 0 || currentRoom.getHotelId().getValue() !== currentHotelId) {
        ...
        currentHotelId = hotel.getId().getValue()
        this.pushHotel(hotels, hotel)
        currentHotelIndex++
      }
